...
from models.transformer.encode_decode.modelper import EncoderDecoder
...
#from  trainer.mydataset import  prepare_dataset
...
class TransformerTrainer(BaseTrainer):
    ...
    def get_model(self, opt, vocab_size, device):
        #vocab_size = len(tokenizer)
        # build a model from scratch or load a model from a given epoch
        if opt.starting_epoch == 1 :
            # define model
            print("get pretrained model form modelper")
            model = EncoderDecoder()
        else:
            # Load modelc
            file_name = os.path.join(self.save_path, f'checkpoint/model_{5}.pt')
    ...
    def train_epoch(self, dataloader, model,loss_compute, device):
        ...
        for i, batch in enumerate(ul.progress_bar(dataloader, total=len(dataloader))):
            ...
            # print(trg_mask[0],trg_mask[1])
            enc_mask=torch.sign(src)
            dec_mask=torch.sign(trg)
            #print(dec_mask,trg_mask)
            lm_labels=deepcopy(trg)
            lm_labels.masked_fill(lm_labels == 0, -100)
            # Compute loss
            adv=True
            if adv:
                out,cont_loss,permodels = model.forward(src, trg, src_mask, trg_mask,enc_mask,dec_mask,adv,lm_labels)
                #print(cont_loss,out)
            else:
                out,permodels = model.forward(src, trg, src_mask, trg_mask,enc_mask,dec_mask,adv,lm_labels)
                cont_loss=0
            # print()
            loss = loss_compute(out, trg_y, ntokens,cont_loss)
            total_tokens += ntokens
            
            loss=float(loss)+float(cont_loss)
            total_loss += loss

        loss_epoch = total_loss / total_tokens

        return loss_epoch,permodels

    def validation_stat(self, dataloader, model, loss_compute, device):
        pad = cfgd.DATA_DEFAULT['padding_value']
        total_loss = 0
        ...
        for i, batch in enumerate(ul.progress_bar(dataloader, total=len(dataloader))):
            ...
            with torch.no_grad():
                ...
                adv=False
                out,per_models = model.forward(src, trg, src_mask, trg_mask,enc_mask,dec_mask,adv,lm_labels)
                loss = loss_compute(out, trg_y, ntokens,None)
    ...
    def train(self, opt):
        ...
        dataloader_validation = self.initialize_dataloader(opt.data_path, opt.batch_size, vocab, 'test')


        device = ut.allocate_gpu()

        model = self.get_model(opt, vocab_size, device)
        ...
        for epoch in range(opt.starting_epoch, opt.starting_epoch + opt.num_epoch):
            ...
            model.train()
            loss_epoch_train,permodels = self.train_epoch(dataloader_train,
                                                       model,       SimpleLossCompute(model.generator,
                                                                 criterion,
                                                                 optim), device)

            self.LOG.info("Training end")
            self.save(permodels, optim, epoch, vocab_size, opt)
