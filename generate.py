...
import models.dataset_dmo as md
...
class GenerateRunner():
    ...
    def generate(self, opt):
        ...
        if opt.model_choice == 'transformer':
            model = EncoderDecoderold.load_from_file(file_name)
            model.to(device)
            model.eval()
        ...
        for j, batch in enumerate(ul.progress_bar(dataloader_test, total=len(dataloader_test))):
            ...
            smiles,num_valid_batch_total= self.sample(opt.model_choice, model, src, src_mask,
                                                                       source_length,
                                                                       opt.decode_type,
            ...
            sample_time.extend(num_valid_batch_total)
        ...
        data_sorted['Sample_times']=sample_time
    ...
    def sample(self, model_choice, model, src, src_mask, source_length, decode_type, num_samples=10,
        ...
        return smiles_list,num_valid_batch_total
